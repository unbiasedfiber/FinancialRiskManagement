...
1. future value
2. present value
3. fixed coupon bond valuation
4. taylor expansion
5. Duration as exposure (macaulay duration, modified duration)
6. price change (taylor expansion)
7. consol (bond duration upper bound)
8. portflio duration
...
class Bond(object):
    ...
    def bond_statistics(self):
        ...
        '''.format(self.type, self.fv, self.y, self.cp, self.p, self.t,
                    self.present_value, self.macaulay_duration, self.modified_duration,
                    self.dollar_duration, self.dollar_bp, self.convexity, self.convexity_per_price)
>>>>>>> e09ce9c54fcdd6f211c8657777db6de3212a576c
